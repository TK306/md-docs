...
- **フロントマター** (`SPEC-FM-001`): HTML コメント形式（先頭に `<!--`、末尾に `-->`）でキーバリューを1行ずつ記述します。例: `<!--\nkey: value\n-->`。パーサは各行を `':'` で分割して `Document.front_matter: dict[str,str]` として保持します。
- **フロントマター伝播** (`SPEC-FM-002`): パースした `Document.front_matter` はユースケース経由でモデル生成に伝播されることを要求します。具体的には `ConvertFileUsecase.load_model_from_path` はパース結果の `Document.front_matter` を `DocConvertible.from_nodes(..., front_matter=...)` に渡します。具象モデルは `from_cursor` または `from_nodes` 側でこのフロントマターを参照できること。
...
- **ヘッダ/行の列数整合** (`SPEC-TABLE-002`): テーブルのヘッダセル数と各データ行のセル数は原則一致することを推奨します。実装は行単位のセル数検査を行いますが、ヘッダとの厳密な一致を要求する場合はユースケース側で検証するか、`Table` に行長検証メソッドを追加して厳格に扱ってください。
...
- **ドメイン変換契約（ポート）** (`SPEC-DOMAIN-001`) — ユーザ側は `DocConvertible` を実装してドメイン↔ノード変換を定義します。
	...
	- **フロントマターの出力フック（追記）**: モデルがフロントマターをファイルへ保存したい場合、`DocConvertible.to_front_matter() -> dict[str,str]` を実装することができます。未実装の場合は空辞書が利用されます。
...
- **レンダラ整形責務** (`SPEC-RENDER-002`): `mdformat` による最終整形はアダプタ層で一度だけ行うことを要求します。ドメイン側の IR→Markdown 変換は副作用を持たない純粋処理とし、アダプタ（例: `MarkdownRendererAdapter`）が最終的に `mdformat` を呼んで出力を整形すること。二重整形は避けてください。
